...
export const getAuditLogs = async (
  ...
  try {
    ...
    const paymentEvents =
      (countMap["payment_confirmed"] ?? 0) +
      (countMap["payment_reversed"] ?? 0);
    const permitEvents =
      (countMap["permit_issued"] ?? 0) + (countMap["permit_revoked"] ?? 0);
    const suspicious =
      (countMap["payment_reversed"] ?? 0) + (countMap["login_failed"] ?? 0);
    ...
    // Return directly as a clean object
...
export const getPaymentById = async (
  ...
  try {
    ...
    if (!payment)
      return sendError(res, "Payment not found", "NOT_FOUND", null, 404);
...
export const verifyReceipt = async (
  ...
  try {
    ...
    if (!receipt) {
      return sendError(
        res,
        "Receipt not found or invalid code",
        "NOT_FOUND",
        null,
        404,
      );
...
export const getSuspiciousActivity = async (
  ...
  try {
    ...
    const [largePayments, orphanedPayments, duplicateGroups] =
      await Promise.all([
    ...
    return sendSuccess(res, {
      ...
      flags: {
        ...
        orphanedConfirmations: {
          description:
            "Payments confirmed with no gateway reference and no confirming officer",
