 ...
 */
export const createLevyConfig = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const treasurerId = req.user!.id;
    const {
      name, category, description, amount,
      billingCycle, penaltyRate, effectiveFrom, effectiveTo,
    } = req.body;
 ...
 */
export const listLevyConfigs = async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
export const getLevyConfigById = async (req: Request, res: Response, next: NextFunction) => {
  try {
         let { id } = req.params;
    ...
    if (!config) return sendError(res, 'Levy configuration not found', 'NOT_FOUND', null, 404);
 ...
 */
export const updateLevyConfig = async (req: Request, res: Response, next: NextFunction) => {
  try {
         let { id } = req.params;
    if (Array.isArray(id)) id = id[0];
    const treasurerId = req.user!.id;
 ...
 */
export const toggleLevyConfig = async (req: Request, res: Response, next: NextFunction) => {
  try {
         let { id } = req.params;
    if (Array.isArray(id)) id = id[0];
    const treasurerId = req.user!.id;
 ...
 */
export const getRevenueByOfficer = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const from = queryString(req.query.from);
    const to   = queryString(req.query.to);
    const page  = parseInt(queryString(req.query.page)  ?? '1');
 ...
 */
export const getRevenueByWard = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const from = queryString(req.query.from);
    const to   = queryString(req.query.to);
    const dateRange = buildDateRange(from, to);
    ...
    const wardMap: Record<string, { invoiced: number; collected: number; invoiceCount: number }> = {};
    for (const inv of invoices) {
      const wardId = inv.business?.wardId;
      if (!wardId) continue;
      if (!wardMap[wardId]) wardMap[wardId] = { invoiced: 0, collected: 0, invoiceCount: 0 };
 ...
 */
export const markInvoiceOverdue = async (req: Request, res: Response, next: NextFunction) => {
  try {
         let { id } = req.params;
    if (Array.isArray(id)) id = id[0];
    const treasurerId = req.user!.id;
    ...
    if (['paid', 'cancelled'].includes(invoice.status)) {
      return sendError(res, 'Cannot mark a paid or cancelled invoice as overdue', 'BAD_REQUEST', null, 400);
    }
    ...
    return sendSuccess(res, updated, `Invoice marked overdue${penaltyAmount > 0 ? `. Penalty of ₦${penaltyAmount} applied.` : ''}`);
  } catch (err) { next(err); }
};