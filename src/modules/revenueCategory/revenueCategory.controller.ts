...
export const createCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) {
      return sendError(res, 'A category record matching this name or identifier already exists.', 'CONFLICT', null, 409);
    }
...
export const updateCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const target = await prisma.revenueCategory.findUnique({ where: { id:String(id) } });
    if (!target) {
      return sendError(res, 'Target Revenue Category record not found.', 'NOT_FOUND', null, 404);
    }
...
export const deleteCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!target) {
      return sendError(res, 'Target Category not found.', 'NOT_FOUND', null, 404);
    }

    // Check if records are actively utilizing this category row link
    const usageCount = target._count.levyConfigs + target._count.permitConfigs + target._count.invoices;
    if (usageCount > 0) {
      return sendError(
        res,
    ...
    return sendSuccess(res, { message: 'Revenue matrix category purged successfully.' });
  } catch (err) { next(err); }
};